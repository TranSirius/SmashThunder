...
class GetFolder():
    def __call__(self, user_id):
        ...
        folders = db_session_instance.query(Folder).join(User).filter(User.ID == user_id).all()
        for folder in folders:
            ...
            folder_dict['posts'] = []
            posts = db_session_instance.query(Post).join(Folder).filter(Post.folder_ID == folder_id).all()
            for post in posts:
                post_dict = dict()
...
class GetPost():
    def __call__(self, user_name, folder_name, post_title):
        ...
        post = db_session.query(User).join(Folder).join(Post).filter(User.user_name == user_name).filter(Folder.folder_title == folder_name).filter(Post.post_title == post_title).first()
